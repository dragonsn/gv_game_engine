...
gvt_hash_map<gv_class_info* , HTREEITEM > s_classMap;

HTREEITEM CClassView::FillOneNode(gv_class_info * pcls){

	if (!pcls) return NULL;
	HTREEITEM hClass=NULL;
	if (s_classMap.find(pcls,hClass))
	{
		return hClass;
	}
	HTREEITEM  father=NULL;
	if (	pcls->get_super() 
		&& 	!s_classMap.find(pcls->get_super(),father)
		)
	{
		father=this->FillOneNode(pcls->get_super());
	}
	hClass = m_wndClassView.InsertItem (CString(CStringA(*pcls->get_name_id().string())), 1, 1,father);
	s_classMap.add(pcls,hClass);
	return hClass;
}

void CClassView::FillClassView ()
{
	if (!gv_global::sandbox_mama.try_get() )
	{
		return;
	}
	...
	gv_global::sandbox_mama->get_base_sandbox()->query_classes_derived_from(gv_id("gv_object"),result);
	for ( int i=0; i< result.size(); i++)
	{
		this->FillOneNode(result[i]);
	}
}
