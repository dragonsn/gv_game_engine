...
extern udp::endpoint to_asio(const gv_socket_address& address);
extern gv_socket_address from_asio(const udp::endpoint& end);
}
}
namespace unit_test_asio
{
using namespace boost::asio::ip;
...
void main(gvt_array< gv_string >& args)
{
	...
	udp::endpoint e;
	gv_socket_address a(gv_ip_address_ipv4(127, 2, 3, 4), 12124);
	e = gv_asio::to_asio(a);
	a = gv_asio::from_asio(e);
	return;
	try
	{
		boost::asio::io_service io_service;
		using namespace std; // For atoi.
		server s(io_service, 9998);
		io_service.run();
	}
