...
void main(gvt_array< gv_string >& args)
{
	gv_bool is_server = args.find("server");
	{
		...
		sub_test_timer timer("unit_test_world_2d");
		//============>>INIT=================================================>>
		m_sandbox->register_processor(gv_world_2d::static_class(), gve_event_channel_world);
		gv_global::rnd_opt.m_max_debug_line = 10000;
		gv_global::rnd_opt.m_max_debug_tri = 1000;
		gv_world_2d* my_world = gvt_cast< gv_world_2d >(m_sandbox->get_event_processor(gve_event_channel_world));
		my_world->init(gv_vector2i(map_width, map_height), gv_vector2i(1000, 1000), the_map);
		...
		{ //
			my_entity->add_component(gv_actor_2d::static_class());
		}
